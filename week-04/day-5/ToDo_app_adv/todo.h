...
void add_task(list_t *list, char *description)
{
    if (strcmp(description, "") == 0) {
        ...
        list->array[new_len - 1].checked = 0;
    }
}
...
void list_tasks(list_t *list)
{
    ...
    for (int i = 0; i < list->len; i++) {
        ...
        if(list->array[i].checked == 1) {
            strcpy(checked_state, "X");
        }
        printf("%d - [%s]\t%s\n", i+1, checked_state, list->array[i].description);
    }
}
...
void command_execution(char *instruction, char *input, list_t *list)
{
    ...
    if(strcmp(instruction, "-a") == 0) {
            ...
            strcpy(description, get_description(input));
            check_task(list, description);
    } else if (strcmp(instruction, "-p") == 0) {
            strcpy(description, get_instruction(input));
            strcpy(addtl_info, get_description(input));

            add_prio_task(list, description, addtl_info);
    } else {
 ...
 ***************/
 ...
 void check_task(list_t *list, char *index)
 {
     list->array[atoi(index) -1].checked = 1;
     //test print
     printf("Checked state of %s: %d", list->array[atoi(index)-1].description, list->array[atoi(index)-1].checked);
 }

 /**********************
 *Add task with priority
 ***********************/
void add_prio_task(list_t *list, char *description, char *prio)
{
    if (strcmp(description, "") == 0) {
        printf("Unable to add: No task is provided\n");
    } else {
        ...
        strcpy(list->array[new_len - 1].description, description);
        list->array[new_len - 1].checked = 0;
        list->array[new_len - 1].priority = atoi(prio);
    }
}
